/**
 * Small synchronous SHA-256 utility for stable machine-contract fingerprints.
 *
 * This implements FIPS PUB 180-4 over UTF-8 bytes. It is a deterministic serialization utility,
 * not semantic or cryptographic authority for CSS behavior. Keeping it internal avoids a Node
 * `crypto` dependency and preserves the synchronous browser-safe core API.
 */
...
export function sha256(value: string): string {
  ...
  for (let blockOffset = 0; blockOffset < paddedLength; blockOffset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choose + (ROUND_CONSTANTS[index] as number) + (schedule[index] as number)) >>>
        0;
